...
def make_dynamics(params: PulleyParams, inputs: Inputs):
    ...
    def f(x):
        ...
        tau1 = inputs.tau1; tau2 = inputs.tau2; tau3 = inputs.tau3; F1 = params.F1; F2 = params.F2
        ...
        S1 = max(0,params.k_eff1(s1)*s1); S2 = max(0, params.k_eff2(s2)*s2); S3 = max(0, params.k_eff3(s3)*s3)

        ddphi1 = (tau1 - c1*dphi1 - r1*S1) / I1
        ddphi2 = (tau2 - c2*dphi2 - r2*S2) / I2
        ddphi3 = (tau3 - c3*dphi3 - r3*S3) / I3
        ddtheta1 = (r4*(S3 - S2 - S1) + F1*l1 - c4*dtheta1) / I4
        # print(ddtheta1, dtheta1)
        ddtheta2 = (r5*(S2 - S1) + F2*l2 - c5*dtheta2) / I5
