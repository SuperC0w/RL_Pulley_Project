...
class PulleyEnvGym(gym.Env):
    ...
    def __init__(self, dt=0.001, max_steps=10000, render_mode=None, seed=None, action_repeat=10, max_action_diff1=0.001, max_action_diff2=0.0005):
        super().__init__()
        self.dt = float(dt)
        ...
        self.max_action_diff1 = max_action_diff1
        self.max_action_diff2 = max_action_diff2
        self.u_max = self.sim.params.tau_max1
        self.pulley_radius = self.sim.params.r1
        self.u_coact_max = self.u_max - self.max_action_diff2/2
        self.u_coact_min = (0 + max_action_diff1/2 + max_action_diff2/4)*2
        ...
        self.theta1_goal = 0
        self.theta2_goal = 0
        # Goal position random ranges
        self.theta1_goal_init_range = (-np.pi/2, np.pi/2)
        self.theta2_goal_init_range = (-np.pi/2, np.pi/2)
        self.coact_goal = 0
        self.coact_goal_init_range = (self.u_coact_min, self.u_coact_max)
        self.goal_dim = 6 # since we are using cos theta, sin theta to encode the goal position
        self.random_theta_flag = True
        self.theta1_init_range = (-np.pi/2, np.pi/2)
        self.theta2_init_range = (-np.pi/2, np.pi/2)
        self.random_dtheta_flag = True
        self.dtheta1_init_range = (-np.deg2rad(45.0), np.deg2rad(45.0))
        self.dtheta2_init_range = (-np.deg2rad(45.0), np.deg2rad(45.0))
        ...
        base_obs_dim = 10  # [theta1, dtheta1, theta2, dtheta2, phi1, dphi1, phi2, dphi2, phi3, dphi3]
        ...
        low = np.array([-self.max_action_diff1, -self.max_action_diff2], dtype=np.float32)
        high = np.array([self.max_action_diff1, self.max_action_diff2], dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.random_theta_flag:
            theta1 = rng.uniform(*self.theta1_init_range)
            theta2 = rng.uniform(*self.theta2_init_range)
        else:
            theta1 = 0
            theta2 = 0
        if self.random_dtheta_flag:
            dtheta1 = rng.uniform(*self.dtheta1_init_range)
            dtheta2 = rng.uniform(*self.dtheta2_init_range)
        else:
            dtheta1 = 0
            dtheta2 = 0
        x = base_obs.astype(np.float32, copy=True)
        x[:] = 0.0
        x[0] = np.float32(theta1)
        x[1] = np.float32(dtheta1)
        x[2] = np.float32(theta2)
        x[3] = np.float32(dtheta2)
        # winding up motor pulleys when setting initial positions for theta1 and theta2
        x[4] = np.float32(-theta1*self.sim.params.r4/self.sim.params.r1 - theta2*self.sim.params.r5/self.sim.params.r1)
        x[6] = np.float32(-theta1*self.sim.params.r4/self.sim.params.r2 + theta2*self.sim.params.r5/self.sim.params.r2)
        x[8] = np.float32(theta1*self.sim.params.r4/self.sim.params.r3)
        self.sim._x = x
        ...
        # sample goal angle θ*
        self.theta1_goal = float(rng.uniform(*self.theta1_goal_init_range))
        self.theta2_goal = float(rng.uniform(*self.theta2_goal_init_range))
        ...
        obs = self._observe()
        info = {"t": 0.0, "theta1_goal": self.theta1_goal, "theta2_goal": self.theta2_goal,
                "coact_goal": self.coact_goal,"F": float(self.sim.params.F)}
        return obs, info
    
    def _map_reparam_to_torques(self, a: np.ndarray) -> np.ndarray:
        """
        Map reparameterized action [u_coact (N), delta (Nm)] to torques [tau1, tau2, tau3] (Nm),
        enforcing bounds so both torques are within [0, u_max] and |tau1 - tau2| <= 2*|delta|.
        """
        delta1 = float(a[0])
        delta2 = float(a[1])

        base = self.coact_goal
    ...
    def step(self, action):
        ...
        for i in range(self.action_repeat):
            ...
            *_, info = self.sim.step(action)
            ...
            total_reward -= (self._vel_safety_penalty(obs[1]) + self._vel_safety_penalty(obs[3]))
        ...
        info = {
            "t": t,
            "theta1_goal": self.theta1_goal,
            "theta2_goal": self.theta2_goal,
    ...
    def _goal_vec(self):
        # Encode goal angle as [cos theta1, sin theta2, e_theta1, cos theta2, sin theta2, e_theta2]
        e_theta1 = self._wrap_pi(self.sim._obs()[0] - self.theta1_goal)
        e_theta2 = self._wrap_pi(self.sim._obs()[2] - self.theta2_goal)
        return np.array([np.cos(self.theta1_goal), np.sin(self.theta1_goal), e_theta1,
                         np.cos(self.theta2_goal), np.sin(self.theta2_goal), e_theta2], dtype=np.float32)
    ...
    def _reward(self, obs, action):
        ...
        theta1, dtheta1, e_theta1 = float(obs[0]), float(obs[1]), float(obs[12])
        theta2, dtheta2, e_theta2 = float(obs[2]), float(obs[3]), float(obs[15])

        # weights (tune as needed)
        w_e  = 100         # angle error

        cost = w_e*e_theta1*e_theta1 + w_e*e_theta2*e_theta2

        reward = -cost
        
        terms = {"e2_1": e_theta1*e_theta1, "e2_2": e_theta2*e_theta2}
        
        return reward, terms

    def _terminated(self, obs, action):
        theta1, dtheta1 = float(obs[0]), float(obs[1])
        theta2, dtheta2 = float(obs[2]), float(obs[3])
        
        # TESTING->checking to see if both joints are unable to get to desired position
        if not self._coact_success(action):
            print("NANI")

        # Success check
        if self._theta_success(obs) and self._coact_success(action):
            # TESTING->checking success streak
        ...
        # Angle limit check
        if (abs(self._wrap_pi(theta1)) > self.safety_limits["angle"] or 
                abs(self._wrap_pi(theta2)) > self.safety_limits["angle"]):
            return True, "angle_limit"
        
        # Soft velocity check
        if (abs(dtheta1) > self.safety_limits["vel"] or abs(dtheta2) > self.safety_limits["vel"]):
    ...
    def _theta_success(self, obs):
        theta1, dtheta1 = float(obs[0]), float(obs[1])
        theta2, dtheta2 = float(obs[2]), float(obs[3])
        e_theta1 = self._wrap_pi(theta1 - self.theta1_goal)
        e_theta2 = self._wrap_pi(theta2 - self.theta2_goal)
        # TESTING->checking to see if both joints are unable to get to desired position
        # if abs(e_theta1) < self.success_band["angle"]:
        #     print("theta 1 success")
        # elif abs(e_theta2) < self.success_band["angle"]:
        #     print("theta 2 success")
        return (abs(e_theta1) < self.success_band["angle"] and abs(e_theta2) < self.success_band["angle"])
