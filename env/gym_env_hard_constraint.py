...
class PulleyEnvGym(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # TESTING->trying out different way to enforce the velocity limit
        self._vel_violate_steps = 0
    ...
    def step(self, action):
        ...
        action = self._project_action_pair(action, self.max_action_diff)
        # print(action)
    ...
    def _vel_safety_penalty(self, dtheta: float) -> float:
        ...
        # TESTING->setting a limit to how high the velocity can be for the calculation of the penalty
        # ad = abs(float(dtheta))
    ...
    def _project_action_pair(self, action: np.ndarray, max_diff: float) -> np.ndarray:
        """
        Project (a1, a2) into the feasible set:
        0 <= a_i <= u_max and |a1 - a2| <= max_diff
        Minimizes L2 distance to the original action, in closed form.
        """
        low, high = 0.0, float(self.u_max)
        a1, a2 = float(action[0]), float(action[1])

        # 1) Respect box bounds first
        a1 = np.clip(a1, low, high)
        a2 = np.clip(a2, low, high)

        # 2) If already within the band, done
        diff = abs(a1 - a2)
        if diff <= max_diff:
            return np.array([a1, a2], dtype=np.float32)

        # 3) Project onto the line segment a1-a2 = s * max_diff (closest point in L2)
        m = 0.5 * (a1 + a2)                 # midpoint
        s = 1.0 if (a1 >= a2) else -1.0     # which side is larger
        half = max_diff * 0.5

        # Keep within [low, high] while imposing the half-difference
        # Max half-diff we can sustain without leaving the box at this midpoint:
        half_bound = min(high - m, m - low)
        half = min(half, half_bound)

        a1_p = m + s * half
        a2_p = m - s * half

        # (Should already be within bounds, but clip for safety)
        a1_p = np.clip(a1_p, low, high)
        a2_p = np.clip(a2_p, low, high)
        return np.array([a1_p, a2_p], dtype=np.float32)